...
parser.add_argument('-f', action='store', dest='linf', help='the file name for the matrix text-file output from UMLS similarity')
...
parser.add_argument('-d','--dir',dest='res_dir',help='Results directory. If none provided, results will be saved in the same directory as the lin matrix file')
...
df = pd.read_table(args.linf,delimiter='\t',header=0)#,na_values="NULL")
